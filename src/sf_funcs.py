...
def compute_sf_fast(data, lags, powers=[2], return_missing=False):
    ...
    This function operates on 1D data (single column). For multi-component analysis,
    call this function separately for each component and average the results.

    Parameters:
        data : pd.Series or pd.DataFrame with single column (N x 1)
        lags : iterable of int
        powers : iterable of float
        return_missing : bool, whether to return gp array

    Returns:
        sf_results : dict {order: np.ndarray of structure function values}
        gp : np.ndarray (if return_missing=True)
    """
    # Convert to Series if DataFrame with single column
    if isinstance(data, pd.DataFrame):
        if data.shape[1] != 1:
            raise ValueError("Input data must be 1D (single column)")
        data = data.iloc[:, 0]

    lags = np.asarray(lags, dtype=int)
    sf_results = {p: np.empty(len(lags)) for p in powers}
    gp = np.empty(len(lags)) if return_missing else None
    ...
    for idx, lag in enumerate(lags):
        ...
        diffs = shifted_data - data

        # Find valid (non-NaN) differences
        valid = ~np.isnan(diffs)
        n_valid = valid.sum()

        # Compute structure function for each power
        for p in powers:
            valid_data = diffs[valid]
            sf_results[p][idx] = (
                np.nanmean(valid_data**p) if valid_data.size > 0 else np.nan
            )

        if return_missing:
            gp[idx] = 100 * (1 - n_valid / (N - lag))
...
def plot_sample(
    ...
    sparsities = [df["gp_overall"].values[0] for df in other_outputs_plot]
    ...
    for i in range(n):
        missing = other_outputs_plot[i]["gp_overall"].values[0]
...
def plot_error_trend_scatter(
    ...
    sfn_mape = bad_outputs_df.groupby("gp_overall")["sf_2_pe"].agg(
        lambda x: np.mean(np.abs(x))
    )

    sfn_mape_i = interp_outputs_df.groupby("gp_overall")["sf_2_pe"].agg(
        lambda x: np.mean(np.abs(x))
    )

...
def per_spectrum(ar1, phys_dims=None):
    ...
    if phys_dims is None:
        phys_dims = [2.0 * np.pi for _ in range(len(ar1.shape))]
    kvec = []
    dx = []
    dk = []
    for i, N in enumerate(ar1.shape):
        dx.append(phys_dims[i] / N)
        dk.append(2.0 * np.pi / (N * dx[i]))
        kvec.append(np.fft.fftshift(np.fft.fftfreq(N)) * 2.0 * np.pi / dx[i])
...
def integrate_spectrum(kvec, mspec, phys_dims):
    ...
    min_k = 2.0 * np.pi / phys_dims[0]
    bins, ispec, istd = bin_data(
        kmesh,
        mspec,
        bin_func=np.nansum,
        cut_excess=True,
...
def bin_data(
    ...
    if num_bins is None:
        ...
        nbins = num_bins
    bins, bin_widths, be = get_bins(
        min_bin, max_bin, nbins, max_half_bin_width, log_space
    )
    ...
    ar1d, bin_edges, _ = binned_statistic(
        nar.ravel(), car.ravel(), bins=be, statistic=bin_func
    )
    if bin_loc == "center":
        # Set the bins to the mid point of the bin edges
        bins1d = (bin_edges[1:] + bin_edges[:-1]) / 2.0
    elif bin_loc == "true_center":
        # Set the bins to the mid point of the actual `nar` data
        new_bins = []
        for i in range(len(bin_edges) - 1):
            bmin, bmax = bin_edges[i].round(decimals=10), bin_edges[i + 1].round(
                decimals=10
            )
            mask = np.where(np.logical_and(nar >= bmin, nar < bmax))
            if nar[mask].size == 0:
                new_bins.append((bmin + bmax) / 2.0)
            else:
                new_bins.append((np.nanmin(nar[mask]) + np.nanmax(nar[mask])) / 2.0)
    ...
    if max_half_bin_width:
        ...
        if max_half_bin_width is not None:
            width = np.minimum(width, 2.0 * max_half_bin_width)
    if norm_bin_size:
        ar1d = ar1d / width
    if nan_small:
        # Compute the counts, so we can ignore bad statistics
        cts, _, _ = binned_statistic(
            nar.ravel(), car.ravel(), bins=be, statistic="count"
        )
        mask = cts <= 1
        ar1d[mask] = np.nan
    if cut_excess:
...
def get_bins(min_bin, max_bin, nbins, max_half_bin_width=None, log_space=False):
    ...
    bin_width = (upper_bins - lower_bins) / 2.0
    if max_half_bin_width is not None:
        bin_width = np.minimum(bin_width, max_half_bin_width)
    bins = upper_bins - bin_width
    bin_edges = np.unique(
        np.concatenate((bins - bin_width, bins + bin_width)).round(decimals=10)
    )
...
def debias(est_alpha, b, D):
    ...
    spec_bias = (
        ...
        * sp_b ** (sp_beta - sp.S(1))
        * (
            sp.gamma(sp_D / sp.S(2))
            * sp.gamma((sp.S(3) - sp_beta) / sp.S(2))
            / sp.gamma(sp_D / sp.S(2) + sp_beta / sp.S(2) - sp.Rational(1, 2))
        )
    )
